...
@section('page_content')
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Add Students in Course({{ $courses[0]->name }})</h1>
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-body">
              <div class="table-responsive">
				<form method="post" action="{{ url('/admin/seleted_students_add_to_course') }}">
				@csrf
                  ...
                  <tbody>
					...
					@foreach($students as $data)
						...
						@foreach($students_courses as $students_course)
							...
							@if($students_course->user_id==$data->id)
								
								@if($students_course->status==1)
									<?php $cond=false; ?>
								@endif
						...
						@if($cond)
						<tr>
						  <td> <input type="checkbox" name="selected_students[]" class="selected_students" value="{{ $data->id }}" /> </td>
						  <td>{{ $data->id }}</td>
						  <td>{{ $data->student }}</td>
						  <td>{{ $data->email }}</td>
						</tr>
					...
					<button type="submit" style="" class="btn btn-primary">Add Students</button>
				</form>
              </div>
            </div>
          </div>
